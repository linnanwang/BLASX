/*
 * -- BLASX (version 1.0.0) --
 * @generated d Mon Jun 15 16:57:40 2015
 */
                 ...
                 double *B, const int ldb)
{
    ...
    switch (type) {
        case 1:
        CPU_BLAS:
            Blasx_Debug_Output("Calling cblas_dtrsm\n ");
            if (cpublas_handle == NULL) blasx_init(CPU);
            if (cblas_dtrsm_p == NULL) blasx_init_cblas_func(&cblas_dtrsm_p, "cblas_dtrsm");
            (*cblas_dtrsm_p)(Order,Side,Uplo,TransA,Diag,M,N,alpha,A,lda,B,ldb);
            break;
        case 2:
            if (cublasXt_handle == NULL) blasx_init(CUBLASXT);
            Blasx_Debug_Output("Calling cublasXtDtrsm\n");
            status = cublasXtDtrsm(cublasXt_handle,
                                   side,   uplo,
                                   transa, diag,
                                   M, N,
                                   (double*)&alpha,
...
void dtrsm_(char *side, char *uplo, char *transa, char *diag,
            ...
            double *B, int *ldb)
{
    Blasx_Debug_Output("Calling dtrsm_ interface\n");
    ...
    cblas_dtrsm(CblasColMajor, Side, Uplo,
                TransA, Diag, *m, *n,
