                 ...
                 const int N, const double *Ap, double *X, const int incX)
{
    ...
    if (cblas_dtpmv_p == NULL) blasx_init_cblas_func(&cblas_dtpmv_p, "cblas_dtpmv");
    Blasx_Debug_Output("Calling cblas_dtpmv\n ");
    (*cblas_dtpmv_p)(Order,Uplo,TransA,Diag,N,Ap,X,incX);
}

/* f77 interface */
void dtpmv_(char *uplo, char *transa, char *diag, int *n, double *Ap, double *X, int *incx)
{
    ...
    cblas_dtpmv(CblasColMajor, Uplo,
                TransA, Diag,
                *n, Ap, X, *incx);
}
