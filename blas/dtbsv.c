                 ...
                 double *X, const int incX)
{
    ...
    if (cblas_dtbsv_p == NULL) blasx_init_cblas_func(&cblas_dtbsv_p, "cblas_dtbsv");
    Blasx_Debug_Output("Calling cblas_dtbsv\n ");
    (*cblas_dtbsv_p)(Order,Uplo,TransA,Diag,N,K,A,lda,X,incX);
}

/* f77 interface */
void dtbsv_(char *uplo, char *transa, char *diag, int *n, int *k, double *A, int *lda, double *X, int *incx)
{
    ...
    cblas_dtbsv(CblasColMajor, Uplo,
                TransA, Diag,
                *n, *k, A, *lda,
                X,*incx);
}
