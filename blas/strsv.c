                 ...
                 const int incX)
{
    ...
    if (cblas_strsv_p == NULL) blasx_init_cblas_func(&cblas_strsv_p, "cblas_strsv");
    Blasx_Debug_Output("Calling cblas_strsv\n ");
    (*cblas_strsv_p)(Order,Uplo,TransA,Diag,N,A,lda,X,incX);
}

/* f77 interface */
void strsv_(char *uplo, char *transa, char *diag, int *n, float *A, int *lda,
            float *X, int *incx)
{
    Blasx_Debug_Output("Calling strsv_ interface\n");
    ...
    cblas_strsv(CblasColMajor, Uplo,
                TransA, Diag,
                *n, A, *lda, X,
                *incx);
}
