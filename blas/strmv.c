                 ...
                 float *X, const int incX)
{
    ...
    if (cblas_strmv_p == NULL) blasx_init_cblas_func(&cblas_strmv_p, "cblas_strmv");
    Blasx_Debug_Output("Calling cblas_strmv\n ");
    (*cblas_strmv_p)(Order,Uplo,TransA,Diag,N,A,lda,X,incX);
}

/* f77 interface */
void strmv_(char *uplo, char *transa, char *diag, int *n, float *A, int *lda,
            float *X, int *incx)
{
    ...
    cblas_strmv(CblasColMajor, Uplo,
                TransA, Diag,
                *n, A, *lda,
                X, *incx);
}
