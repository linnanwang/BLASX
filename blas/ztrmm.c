                 ...
                 blasxDoubleComplex *B, const int ldb)
{
    ...
    switch (type) {
        case 1:
            Blasx_Debug_Output("Calling cblas_ztrmm\n ");
            if (cpublas_handle == NULL) blasx_init(CPU);
            if (cblas_ztrmm_p == NULL) blasx_init_cblas_func(&cblas_ztrmm_p, "cblas_ztrmm");
            (*cblas_ztrmm_p)(Order,Side,Uplo,TransA,Diag,M,N,alpha,A,lda,B,ldb);
            break;
        default:
            break;
    }
            ...
            blasxDoubleComplexF77 *B, int *ldb)
{
    ...
    cblas_ztrmm(CblasColMajor, Side,
                Uplo, TransA,
                Diag, *m, *n,
