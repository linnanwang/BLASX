                 ...
                 void *X, const int incX)
{
    ...
    if (cblas_ztrmv_p == NULL) blasx_init_cblas_func(&cblas_ztrmv_p, "cblas_ztrmv");
    Blasx_Debug_Output("Calling cblas_ztpmv\n ");
    (*cblas_ztrmv_p)(Order,Uplo,TransA,Diag,N,A,lda,X,incX);
}

/* f77 interface */
void ztrmv_(char *uplo, char *transa, char *diag, int *n, double *A, int *lda,
                      double *X, int *incx)
{
    ...
    cblas_ztrmv(CblasColMajor, Uplo,
                TransA, Diag,
                *n, A, *lda,
                X, *incx);
}
