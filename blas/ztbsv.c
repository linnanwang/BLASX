                 ...
                 void *X, const int incX)
{
    ...
    if (cblas_ztbsv_p == NULL) blasx_init_cblas_func(&cblas_ztbsv_p, "cblas_ztbsv");
    Blasx_Debug_Output("Calling cblas_ztbsv\n ");
    (*cblas_ztbsv_p)(Order,Uplo,TransA,Diag,N,K,A,lda,X,incX);
}

/* f77 interface */
void ztbsv_(char *uplo, char *transa, char *diag, int *n, int *k, double *A, int *lda, double *X, int *incx)
{
    ...
    cblas_ztbsv(CblasColMajor, Uplo,
                TransA, Diag,
                *n, *k, A, *lda,
                X,*incx);
}
