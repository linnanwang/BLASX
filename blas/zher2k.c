                  ...
                  const int ldc)
{
    ...
    switch (type) {
        ...
        CPU_BLAS:
            ...
            if (cblas_zher2k_p == NULL) blasx_init_cblas_func(&cblas_zher2k_p, "cblas_zher2k");
            Blasx_Debug_Output("Calling cblas_zher2k\n ");
            (*cblas_zher2k_p)(Order,Uplo,Trans,N,K,alpha,A,lda,B,ldb,beta,C,ldc);
            break;
        case 2:
            if (cublasXt_handle == NULL) blasx_init(CUBLASXT);
            Blasx_Debug_Output("Calling cublasXtZher2k\n");
            status = cublasXtZher2k(cublasXt_handle,
                                    uplo, trans,
                                    N, K,
