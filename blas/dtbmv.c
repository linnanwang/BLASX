                 ...
                 double *X, const int incX)
{
    ...
    if (cblas_dtbmv_p == NULL) blasx_init_cblas_func(&cblas_dtbmv_p, "cblas_dtbmv");
    Blasx_Debug_Output("Calling cblas_dtbmv\n ");
    (*cblas_dtbmv_p)(Order,Uplo,TransA,Diag,N,K,A,lda,X,incX);
}

/* f77 interface */
void dtbmv_(char *uplo, char *transa, char *diag, int *n, int *k, double *A, int *lda, double *X, int *incx)
{
    ...
    cblas_dtbmv(CblasColMajor, Uplo,
                TransA, Diag,
                *n, *k, A, *lda,
                X,*incx);
}
