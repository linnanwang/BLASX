                 ...
                 void *X, const int incX)
{
    ...
    if (cblas_ctbmv_p == NULL) blasx_init_cblas_func(&cblas_ctbmv_p, "cblas_ctbmv");
    Blasx_Debug_Output("Calling cblas_ctbmv\n ");
    (*cblas_ctbmv_p)(Order,Uplo,TransA,Diag,N,K,A,lda,X,incX);
}

/* f77 interface */
void ctbmv_(char *uplo, char *transa, char *diag, int *n, int *k, float *A, int *lda, float *X, int *incx)
{
    Blasx_Debug_Output("Calling ctbmv_ interface\n");
    ...
    cblas_ctbmv(CblasColMajor, Uplo,
                TransA, Diag,
                *n, *k, A, *lda,
                X,*incx);
}
