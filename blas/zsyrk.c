                 ...
                 const blasxDoubleComplex *beta, blasxDoubleComplex *C, const int ldc)
{
    ...
    switch (type) {
        case 1:
        CPU_BLAS:
            Blasx_Debug_Output("Calling cblas_zsyrk\n ");
            if (cpublas_handle == NULL) blasx_init(CPU);
            if (cblas_zsyrk_p == NULL)  blasx_init_cblas_func(&cblas_zsyrk_p, "cblas_zsyrk");
            (*cblas_zsyrk_p)(Order,Uplo,Trans,N,K,alpha,A,lda,beta,C,ldc);
            break;
        case 2:
            if (cublasXt_handle == NULL) blasx_init(CUBLASXT);
            Blasx_Debug_Output("Calling cublasXtZsyrk\n");
            status = cublasXtZsyrk(cublasXt_handle,
                                   uplo, trans,
                                   N, K,
...
{
    Blasx_Debug_Output("Calling zsyrk_ interface\n");
    ...
    cblas_zsyrk(CblasColMajor, Uplo,
                Trans, *n, *k,
