                 ...
                 double *B, const int ldb)
{
    ...
    switch (type) {
        case 1:
            Blasx_Debug_Output("Calling cblas_dtrmm\n ");
            if (cpublas_handle == NULL) blasx_init(CPU);
            if (cblas_dtrmm_p == NULL) blasx_init_cblas_func(&cblas_dtrmm_p, "cblas_dtrmm");
            (*cblas_dtrmm_p)(Order,Side,Uplo,TransA,Diag,M,N,alpha,A,lda,B,ldb);
            break;
        default:
            break;
    }
...
/* f77 interface */
void dtrmm_(char *side, char *uplo, 
            char *transa, char *diag,
            int *m, int *n,
            double *alpha, double *A, int *lda,
            double *B, int *ldb)
{
    Blasx_Debug_Output("Calling dtrmm_ interface\n");
    ...
    cblas_dtrmm(CblasColMajor, Side,
                Uplo, TransA,
                Diag, *m, *n,
