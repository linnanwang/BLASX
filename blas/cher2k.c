                  ...
                  const int ldc)
{
    ...
    switch (type) {
        ...
        CPU_BLAS:
            ...
            if (cblas_cher2k_p == NULL) blasx_init_cblas_func(&cblas_cher2k_p, "cblas_cher2k");
            Blasx_Debug_Output("Calling cblas_cher2k\n ");
            (*cblas_cher2k_p)(Order,Uplo,Trans,N,K,alpha,A,lda,B,ldb,beta,C,ldc);
            break;
        case 2:
            if (cublasXt_handle == NULL) blasx_init(CUBLASXT);
            Blasx_Debug_Output("Calling cublasXtCher2k\n");
            status = cublasXtCher2k(cublasXt_handle,
                                    uplo, trans,
                                    N, K,
