                 ...
                 blasxFloatComplex *B, const int ldb)
{
    ...
    switch (type) {
        case 1:
            Blasx_Debug_Output("Calling cblas_ctrmm\n ");
            if (cpublas_handle == NULL) blasx_init(CPU);
            if (cblas_ctrmm_p == NULL) blasx_init_cblas_func(&cblas_ctrmm_p, "cblas_ctrmm");
            (*cblas_ctrmm_p)(Order,Side,Uplo,TransA,Diag,M,N,alpha,A,lda,B,ldb);
            break;
        default:
            break;
    }
            ...
            blasxFloatComplexF77 *B, int *ldb)
{
    ...
    cblas_ctrmm(CblasColMajor, Side,
                Uplo, TransA,
                Diag, *m, *n,
