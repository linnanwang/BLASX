                 ...
                 const double beta, double *C, const int ldc)
{
    ...
    switch (type) {
        case 1:
        CPU_BLAS:
            Blasx_Debug_Output("Calling cblas_dsyrk\n ");
            if (cpublas_handle == NULL) blasx_init(CPU);
            if (cblas_dsyrk_p == NULL)  blasx_init_cblas_func(&cblas_dsyrk_p, "cblas_dsyrk");
            (*cblas_dsyrk_p)(Order,Uplo,Trans,N,K,alpha,A,lda,beta,C,ldc);
            break;
        case 2:
            if (cublasXt_handle == NULL) blasx_init(CUBLASXT);
            Blasx_Debug_Output("Calling cublasXtDsyrk\n");
            status = cublasXtDsyrk(cublasXt_handle,
                                   uplo, trans,
                                   N, K,
...
{
    Blasx_Debug_Output("Calling dsyrk_ interface\n");
    ...
    cblas_dsyrk(CblasColMajor, Uplo,
                Trans, *n, *k,
                (double)*alpha, (double*)A, *lda,
                (double)*beta, (double*)C, *ldc);
}
