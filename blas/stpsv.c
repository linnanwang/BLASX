                 ...
                 const int N, const float *Ap, float *X, const int incX)
{
    ...
    if (cblas_stpsv_p == NULL) blasx_init_cblas_func(&cblas_stpsv_p, "cblas_stpsv");
    Blasx_Debug_Output("Calling cblas_stpsv\n ");
    (*cblas_stpsv_p)(Order,Uplo,TransA,Diag,N,Ap,X,incX);
}

/* f77 interface */
void stpsv_(char *uplo, char *transa, char *diag, int *n, float *Ap, float *X, int *incx)
{
    ...
    cblas_stpsv(CblasColMajor, Uplo,
                TransA, Diag,
                *n, Ap, X, *incx);
}
