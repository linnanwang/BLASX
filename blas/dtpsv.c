                 ...
                 const int N, const double *Ap, double *X, const int incX)
{
    ...
    if (cblas_dtpsv_p == NULL) blasx_init_cblas_func(&cblas_dtpsv_p, "cblas_dtpsv");
    Blasx_Debug_Output("Calling cblas_dtpsv\n ");
    (*cblas_dtpsv_p)(Order,Uplo,TransA,Diag,N,Ap,X,incX);
}

/* f77 interface */
void dtpsv_(char *uplo, char *transa, char *diag, int *n, double *Ap, double *X, int *incx)
{
    ...
    cblas_dtpsv(CblasColMajor, Uplo,
                TransA, Diag,
                *n, Ap, X, *incx);
}
