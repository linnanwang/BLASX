                 ...
                 const blasxDoubleComplex *beta, blasxDoubleComplex *C, const int ldc )
{
    ...
    if (type == 0 && (M > 500 || N > 500 || K > 500))    type = 3;
    else                                                 type = 1;
    /*-------------------*/
    ...
    switch (type) {
        ...
        case 3:
            ...
            cudaHostRegister(A,sizeof(cuDoubleComplex)*nrowa*ncola,cudaHostRegisterPortable);
            cudaHostRegister(B,sizeof(cuDoubleComplex)*nrowb*ncolb,cudaHostRegisterPortable);
            cudaHostRegister(C,sizeof(cuDoubleComplex)*nrowc*ncolc,cudaHostRegisterPortable);
#ifdef BENCHMARK
