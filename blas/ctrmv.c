                 ...
                 void *X, const int incX)
{
    ...
    if (cblas_ctrmv_p == NULL) blasx_init_cblas_func(&cblas_ctrmv_p, "cblas_ctrmv");
    Blasx_Debug_Output("Calling cblas_ctrmv\n ");
    (*cblas_ctrmv_p)(Order,Uplo,TransA,Diag,N,A,lda,X,incX);
}

/* f77 interface */
void ctrmv_(char *uplo, char *transa, char *diag, int *n, double *A, int *lda,
                      double *X, int *incx)
{
    Blasx_Debug_Output("Calling ctrmv_ interface\n");
    ...
    cblas_ctrmv(CblasColMajor, Uplo,
                TransA, Diag,
                *n, A, *lda,
                X, *incx);
}
